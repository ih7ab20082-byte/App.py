...
st.markdown("<div class='subtitle'>Advanced Decision Support System for Cirrhosis Stratification, SBP Diagnosis & Fluid Stewardship</div>", unsafe_allow_html=True)
...
<div class='branding-bar'>
    <div class='branding-text'>
        ...
        <strong>Clinical Core:</strong> AASLD & EASL Comprehensive Guidelines, MELD-Na Core, SBP Diagnostic Boundaries, PICD Prevention Protocols
    </div>
</div>
""", unsafe_allow_html=True)
...
# ==============================================================================
with col_input:
    st.markdown("<div class='section-header'>📋 1. Baseline Lab Parameters</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='section-header'>🌊 2. Paracentesis & Fluid Analysis</div>", unsafe_allow_html=True)
    is_paracentesis = st.checkbox("Patient underwent Therapeutic Paracentesis today", value=False)
    ...
    # New Sub-feature: Ascitic Fluid Cytology Guard for SBP
    has_fluid_labs = st.checkbox("Ascitic Fluid Lab Results Available (SBP Screen)", value=False)
    wbc_fluid = st.number_input("Ascitic Fluid WBC (cells/mm³):", min_value=0, max_value=50000, value=600, step=50) if has_fluid_labs else 0
    polymorphs = st.number_input("Polymorphonuclears / Neutrophils (%):", min_value=0, max_value=100, value=55, step=5) if has_fluid_labs else 0
    ...
    he_stage = st.selectbox(
        ...
        ["Grade 0: Normal", 
         ...
         "Grade 2: Lethargy, minimal disorientation, Asterixis present", 
         "Grade 3: Somnolence, responsive to verbal stimuli, gross disorientation", 
         "Grade 4: Coma"]
    )
    ...
    cp_score += 2 if is_paracentesis else 1
    ...
    # SBP Dynamic Math Calculation
    pmn_count = (wbc_fluid * polymorphs) / 100 if has_fluid_labs else 0
...
with col_dashboard:
    ...
    tab_strat, tab_para, tab_he = st.tabs(["📊 Severity Stratification", "💧 Paracentesis & SBP Guard", "🧠 Encephalopathy Protocol"])
    ...
    with tab_strat:
        ...
        if meld_na >= 15:
            st.error(f"⚠️ **High Mortality Risk Alert (MELD-Na ≥ 15):** Evaluation for liver transplantation referral is indicated.")
        else:
        ...
        if creatinine > 1.5 and sodium < 130:
            ...
            st.markdown("""
            * **Action Plan:** Hold all diuretics (Furosemide/Spironolactone). Avoid NSAIDs. Ensure volume expansion with active **20% Albumin**. Consider early initiation of **Terlipressin + Albumin** if criteria for HRS-AKI are fully met.
            """)
        st.markdown("</div>", unsafe_allow_html=True)

    # TAB 2: PARACENTESIS INITIATION & SBP GUARD (UPDATED)
    with tab_para:
        st.markdown("### 🌊 Advanced Fluid Stewardship & SBP Surveillance")
        
        # 1. Paracentesis Volume / Albumin Math Block
        if is_paracentesis:
            st.write(f"📊 **Ascitic Fluid Extracted:** `{ascitic_volume} Liters`")
            if ascitic_volume > 5.0:
                albumin_grams = ascitic_volume * 8.0
                # Volume conversion math helper
                vol_20pct = (albumin_grams / 20) * 100
                st.error("🚨 **LARGE VOLUME PARACENTESIS (LVP) DETECTED:**")
                st.markdown(f"**Target Substitution:** Replace **{albumin_grams:.0f} grams** of intravenous Albumin.")
                st.info(f"🧪 **Nursing Order Guide:** Infuse **{vol_20pct:.0f} mL** of **20% Albumin solution** (equivalent to {vol_20pct/50:.1f} vials of 50mL) post-tap to prevent PICD.")
            else:
                st.warning("Volume ≤ 5 Liters: Albumin substitution is optional but highly recommended if hemodynamic instability is present.")
        else:
            st.info("⚪ Paracentesis volume tracker is inactive.")

        # 2. SBP Screening Block (New Addition)
        if has_fluid_labs:
            st.markdown("---")
            st.write(f"🔬 **Calculated Absolute PMN (Neutrophil) Count:** `{pmn_count:.0f} cells/mm³`")
            if pmn_count >= 250:
                st.error("🚨 **DIAGNOSTIC CRITERIA MET FOR SPONTANEOUS BACTERIAL PERTONITIS (SBP):**")
                st.markdown(f"""
                * **Status:** PMN count is **{pmn_count:.0f} cells/mm³** (Threshold is $\ge 250$).
                * **Immediate Action Plan:** Initiate empiric **Ceftriaxone 2g IV once daily** (or Cotaxime) immediately. Do not wait for fluid culture results.
                * **Renal Protection Rule:** To prevent SBP-associated AKI, administer IV Albumin at **1.5 g/kg on Day 1** and **1.0 g/kg on Day 3** if baseline Creatinine $>1.0$ or Bilirubin $>4.0$.
                """)
            else:
                st.success("🟢 SBP Screen Negative: Absolute PMN count is below 250 cells/mm³.")
    ...
    with tab_he:
        ...
        if "Grade 0" in he_stage:
            ...
            * **Second-Line Addition:** Add **Rifaximin 550 mg PO twice daily** immediately if this is a recurrent episode.
...
st.info("""
**Question 1:** A 54-year-old male with a known history of alcoholic cirrhosis presents to the medical ward with progressive abdominal distension. Labs reveal: Serum Creatinine 0.9 mg/dL, Total Bilirubin 1.8 mg/dL, INR 1.3, Albumin 2.5 g/dL, and Serum Sodium 136 mEq/L. A therapeutic large-volume paracentesis is performed, and 7.5 Liters of clear ascitic fluid are evacuated. 
...
* **Correct Answer for Club Discussion:** **A**. *Rationale:* For any large-volume paracentesis exceeding 5 Liters, guideline-directed medical therapy dictates replacing 6 to 8 grams of albumin per Liter of fluid removed. 7.5 Liters × 8g = 60 grams of active Albumin.
""")

st.markdown("---")

st.info("""
**Question 2:** A 58-year-old female with decompensated cirrhosis presents with low-grade fever and mild abdominal discomfort. A diagnostic paracentesis is performed. Fluid analysis reveals: Total WBC 800 cells/mm³ with 45% Neutrophils (Polymorphonuclears). Serum Creatinine is 1.4 mg/dL and Total Bilirubin is 4.2 mg/dL. 

In addition to initiating empiric intravenous Ceftriaxone, which of the following represents the most appropriate guideline-directed medical therapy to reduce mortality and prevent hepatorenal syndrome?
* **A)** Intravenous Furosemide to accelerate fluid clearance.
* **B)** Intravenous Albumin at a dose of 1.5 g/kg on Day 1, followed by 1.0 g/kg on Day 3.
* **C)** Broadening antibiotic coverage to include Vancomycin and Meropenem immediately.
* **D)** Initiating oral Norfloxacin as immediate active therapy.

* **Correct Answer for Club Discussion:** **B**. *Rationale:* The calculated absolute PMN count is 800 × 0.45 = 360 cells/mm³, confirming a diagnosis of Spontaneous Bacterial Peritonitis (SBP threshold $\ge 250$). For patients with SBP who have a Creatinine $>1.0$ mg/dL or Bilirubin $>4.0$ mg/dL, the administration of high-dose intravenous Albumin according to this specific regimen (1.5g/kg on day 1 and 1g/kg on day 3) significantly decreases the incidence of renal failure and decreases hospital mortality.
""")
