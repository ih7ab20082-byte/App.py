...
st.set_page_config(
    page_title="Al-Ahli Perioperative Fitness Engine v4",
    page_icon="🏥",
    layout="wide",
...
# 4. Main App Tabs Configuration (6 Complete Tabs)
tabs = st.tabs([
    ...
    "📊 Risk Stratification & Pulmonary Models",
...
# ==============================================================================
# TAB 5: 📊 RISK STRATIFICATION MODELS (THE RETURNED TAB FROM SCREENSHOT)
# ==============================================================================
with tabs[4]:
    ...
    with col_r2:
        ...
        # Explicitly building dynamic checkboxes based on findings for the user to see or verify
        st.markdown("##### Underlying Pulmonary Criteria:")
        p_age_check = st.checkbox("Age > 60 years old", value=(patient_age > 60))
        p_spo2_check = st.checkbox("Preoperative SpO2 ≤ 95%", value=(spo2 <= 95))
        p_rr_check = st.checkbox("Tachypnea / Respiratory Rate > 20", value=(rr > 20))
        p_exam_check = st.checkbox("Abnormal Chest Auscultation (Wheezing/Rales)", value=("Clear" not in chest_ausc))
        p_cxr_check = st.checkbox("Abnormal CXR Infiltrates / Congestion", value=("Clear" not in cxr_findings))
        p_emerg_check = st.checkbox("Emergency Surgical Procedure Type", value=is_emergency)
        
        # Calculate score dynamically
        pulm_score = 0
        if p_age_check: pulm_score += 3
        if p_spo2_check: pulm_score += 8
        if p_rr_check: pulm_score += 4
        if p_exam_check: pulm_score += 5
        if p_cxr_check: pulm_score += 6
        if p_emerg_check: pulm_score += 8
        
        if pulm_score >= 45: ppc_class = "High Pulmonary Risk"
        elif 26 <= pulm_score <= 44: ppc_class = "Moderate Pulmonary Risk"
        else: ppc_class = "Low Pulmonary Risk"
        
        st.metric(f"Pulmonary Risk Class (Score: {pulm_score}):", ppc_class)

# ==============================================================================
# TAB 6: GLOBAL MEDICAL FITNESS COMPREHENSIVE RECONCILIATION DASHBOARD
# ==============================================================================
with tabs[5]:
...
# Footer Signature
st.markdown("---")
st.markdown("<div style='text-align: center; color: #9CA3AF; font-size: 11px;'>Al-Ahli Hospital Internal Medicine CDSS Platforms v4 • 2026 • Curated by Dr. Ihab Abbass Abu Hilail</div>", unsafe_allow_html=True)
