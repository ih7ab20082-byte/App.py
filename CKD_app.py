...
with col_input:
    ...
    hb_level = st.number_input("Hemoglobin Level (g/dL):", min_value=4.0, max_value=18.0, value=6.0, step=0.1) # تم تعديل القيمة الافتراضية هنا لتظهر النتيجة مباشرة
...
with col_protocol:
    ...
    # ------------------------------------------------------------------
    # NEW UPDATED SECTION B: ANEMIA CRITICAL TRIGGER (Hb <= 7 Focus)
    # ------------------------------------------------------------------
    st.markdown("<div class='status-box'>", unsafe_allow_html=True)
    st.markdown("### 🩸 CKD Anemia Management Protocol")
    
    if hb_level <= 7.0:
        st.error(f"🚨 **CRITICAL ANEMIA DETECTED (Hb: {hb_level} g/dL):**")
        st.markdown(f"""
        ⚠️ **Immediate Action Required:** Hemoglobin level is within the **restrictive transfusion threshold** ($\le 7\ g/dL$).
        * **Order:** Cross-match and prepare **Packed RBCs Transfusion** immediately.
        * **Clinical Precaution:** Transfuse slowly (e.g., over 3-4 hours per unit) and consider concomitant IV *Furosemide* to prevent **TACO** (Transfusion-Associated Circulatory Overload), given the patient's compromised renal clearance (Stage {g_stage}).
        * **Workup:** Send pre-transfusion samples for Iron Studies (Ferritin, TSAT), Reticulocyte count, and Vitamin B12/Folate.
        """, unsafe_allow_html=True)
    elif 7.0 < hb_level < 10.0:
        st.warning(f"⚠️ **Moderate CKD Anemia (Hb: {hb_level} g/dL):**")
        st.write("* **Action Plan:** Target Hb is **10.0 - 11.5 g/dL**. Rule out iron deficiency first (Target Ferritin > 100 ng/mL, TSAT > 20%).")
        st.write("* **Therapy:** If iron replete, initiate **Erythropoiesis-Stimulating Agent (ESA)** such as Epoetin Alfa.")
    else:
        st.success(f"✅ **Anemia Status:** Hemoglobin ({hb_level} g/dL) is within safe or acceptable targets for conservative management.")
    st.markdown("</div>", unsafe_allow_html=True)

    # ------------------------------------------------------------------
    # SECTION C: POTASSIUM CARD (CRITICAL SAFEGUARD)
    # ------------------------------------------------------------------
    st.markdown("<div class='status-box'>", unsafe_allow_html=True)
    ...
    if potassium > 5.2:
        ...
        st.write("* **Medication Guard:** Review and consider holding **ACE inhibitors / ARBs** and Aldosterone Antagonists if $K^+ > 5.5$.")
    ...
    # ------------------------------------------------------------------
    # SECTION D: ADVANCED CKD-MBD EXPERT ENGINE (Secondary vs Tertiary Split)
    # ------------------------------------------------------------------
    st.markdown("<div class='status-box'>", unsafe_allow_html=True)
    ...
    if pth_level > 150:
        if corrected_ca > 10.2:
            ...
            st.markdown(f"""
            💡 *Clinical Reasoning:* High iPTH (**{pth_level} pg/mL**) accompanied by **Hypercalcemia** (Corrected Calcium: **{corrected_ca:.2f} mg/dL**). 
            
            ⚠️ **Contraindication Alert:** Active Vitamin D (Calcitriol) and Calcium-based binders are <span style='color:red; font-weight:bold;'>STRICTLY CONTRAINDICATED</span> here!
            📋 **Next Steps:** Consider **Calcimimetics (Cinacalcet)** or refer to endocrine surgery for Parathyroidectomy if refractory.
            """, unsafe_allow_html=True)
        else:
    ...
    # ------------------------------------------------------------------
    # SECTION E: WARD DRUG DOSING ENGINE
    # ------------------------------------------------------------------
    st.markdown("<div class='status-box'>", unsafe_allow_html=True)
