...
# Custom Branding and Signature Style - Fixed text contrast for white box
st.markdown("""
    <style>
...
# Fixed Professional Badge / Signature with high contrast text
st.markdown(f"""
...
# ==============================================================================
# RIGHT COLUMN: Clinical Evaluation & Neuroimaging Paths
# ==============================================================================
with col_eval:
    ...
    with st.expander("⏱️ Door-to-Needle Quality Timer", expanded=True):
        ...
        door_time = st.number_input("Emergency Department Arrival to CT/MRI Completion (Minutes):", min_value=1, max_value=180, value=25)
        ...
        if target_tpa_time <= 60:
            ...
            st.error(f"⚠️ Target Delayed: Expected Door-to-Needle exceeds 60 mins. Streamline imaging reading and lab processing immediately!")

    st.markdown("---")
    st.markdown("<div class='section-header'>🔬 2. Diagnostic Neuroimaging Modal</div>", unsafe_allow_html=True)
    
    imaging_type = st.radio("Select Initial Neuroimaging Modality & Findings:", 
    ...
    # A. PATHWAY 1 & 2: ISCHEMIC (CT or Diffusion MRI)
    if "Ischemic" in imaging_type or "Diffusion" in imaging_type:
        
        if "Diffusion" in imaging_type:
            st.info("🧠 **Diffusion-Weighted MRI (DWI) Protocol:** Diffusion restriction confirmed. Highly accurate for hyperacute ischemia even if initial CT was negative.")

        with st.expander("📝 Comprehensive NIHSS Calculator", expanded=True):
            st.caption("Rate the patient across the 11 clinical items to determine stroke severity:")
            ...
            # Clinical Risk Warning for Massive Strokes (NIHSS > 22)
            if total_nihss > 22:
                st.error("⚠️ CRITICAL WARNING: High NIHSS (> 22) indicates a massive hemispheric stroke. Severe risk of post-tPA Hemorrhagic Transformation! Proceed with extreme caution and prioritize urgent Endovascular Thrombectomy (EVT) consultation over systemic thrombolysis if an LVO is present.")

    # B. PATHWAY 3: HEMORRHAGIC STROKE (ICH SCORE)
    else:
...
# ==============================================================================
# LEFT COLUMN: Decision Support & Management Protocols
# ==============================================================================
with col_action:
    ...
    if "Hemorrhagic" in imaging_type:
        ...
        * **Current SBP Status:** {"🔴 Elevated! Start aggressive IV titration." if sbp > 140 else "🟢 Within optimal targets."}
        ...
        1. **Anticoagulation Reversal:** If patient is on Warfarin, give Prothrombin Complex Concentrate (PCC) + Vitamin K 10mg IV. If on DOACs, consider targeted reversal agents.
        ...
        if onset_time <= 4.5:
            if total_nihss > 22:
                st.warning("⚠️ IV tPA Relative Contraindication: Severe stroke profile. High hemorrhage risk. Ensure senior/neurology consensus before running thrombolysis.")
            tpa_eligible = True
    ...
    with st.expander("⏱️ Cardioembolic AF Anticoagulation Timing Guide", expanded=False):
        ...
        try:
            current_nihss_val = total_nihss
        except NameError:
            current_nihss_val = 10
            
        af_nihss = st.slider("Select Patient Baseline NIHSS for AF timing:", 0, 42, int(current_nihss_val))
