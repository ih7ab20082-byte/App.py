...
with col_eval:
    ...
    if "Ischemic" in imaging_type or "Diffusion" in imaging_type:
        with st.expander("📝 Mandatory NIHSS Calculator (Must be completed for treatment logs)", expanded=True):
            st.caption("AHA/ASA Requirement: Complete all 11 items to unlock treatment recommendations.")
...
# ==============================================================================
# LEFT COLUMN: Decision Support Based strictly on Guidelines
# ==============================================================================
with col_action:
    ...
    if "Hemorrhagic" in imaging_type:
        ...
        # For Ischemic Path, check if NIHSS is fully integrated
        if not nihss_calculated:
            ...
            # AHA/ASA strictly relies on NIHSS thresholds for safety
            ...
            # 1. ACUTE INTERVENTION INTERFACES & GUIDELINE-BASED BRIDGING
            if tpa_eligible:
                ...
                # Guideline Bridging Rule: Patient fits LVO criteria (NIHSS >= 6) and is receiving tPA
                if evt_eligible:
                    st.warning("🧠 **AHA/ASA BRIDGING MANDATE ACTIVE:** This patient satisfies criteria for both IV thrombolysis (NIHSS ≤ 22) and mechanical thrombectomy (LVO + NIHSS ≥ 6). **Initiate IV tPA infusion immediately and transfer to the Angio-Suite simultaneously.** Do NOT delay EVT to monitor tPA clinical efficacy.")
                elif "Confirmed LVO" in cta_result and total_nihss < 6:
                    st.info("ℹ️ **Note:** LVO is present, but current NIHSS < 6. Thrombolyse with tPA; monitor closely for acute neurological deterioration which may warrant urgent rescue EVT.")
            
            elif evt_eligible and not tpa_eligible:
                st.info("🎯 **Direct Endovascular Thrombectomy (EVT) Candidate:** Patient is outside the IV tPA window (or has contraindications) but fits extended EVT parameters (LVO + NIHSS ≥ 6 up to 24h). Activate the Angio-Suite immediately.")
            
            elif total_nihss > 22:
                st.error("❌ **Systemic Thrombolysis Excluded:** NIHSS > 22 indicates severe devastating infarct. High danger of parenchymal hematoma. Proceed directly to urgent neurointerventional consultation for potential mechanical option.")
