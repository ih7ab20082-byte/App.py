...
# ==============================================================================
# TAB 3: 🧠 EPILEPSY & THYROID SPECIALTY FOCUS
# ==============================================================================
with tabs[2]:
    ...
    # Live Analysis Logic for Thyroid (FIXED TYPO HERE)
    ...
    if is_thyroid_abnormal or "None" not in thyroid_meds:
        ...
        for med in thyroid_meds:
            if "Levothyroxine" in med:
                st.markdown("<div class='info-box'>📋 <b>Levothyroxine Stewardship:</b> GIVE the normal morning dose with a small sip of water on the day of surgery.</div>", unsafe_allow_html=True)
            if "Carbimazole" in med or "PTU" in med:
                st.markdown("<div class='info-box'>📋 <b>Anti-Thyroid Drug Stewardship:</b> GIVE normal home dose on the morning of surgery to prevent hormone surge under stress.</div>", unsafe_allow_html=True)
    ...
    if last_seizure != "Patient does NOT have Epilepsy":
        ...
        for aed in selected_aeds:
            if "Levetiracetam" in aed:
                st.markdown("<div class='specialty-box'>🧠 <b>Levetiracetam (Keppra) Comprehensive Approach:</b><br>• <b>Morning of Surgery:</b> GIVE regular morning dose orally.<br>• <b>NPO Bridging Phase:</b> Convert to <b>IV Keppra at a 1:1 ratio</b> (e.g., 500mg PO BID changes to 500mg IV BID over 15 minutes).</div>", unsafe_allow_html=True)
            if "Sodium Valproate" in aed:
                st.markdown("<div class='specialty-box'>🧠 <b>Sodium Valproate (Depakine) Comprehensive Approach:</b><br>• <b>Morning of Surgery:</b> GIVE regular home dose.<br>• <b>NPO Bridging Phase:</b> Convert to <b>IV Depakine at a 1:1 ratio</b>. Check pre-op platelet count meticulously.</div>", unsafe_allow_html=True)
            if "Carbamazepine" in aed:
                st.markdown("<div class='specialty-box' style='border-color: #B45309; background-color: #FFFBEB; color: #78350F;'>🧠 <b>Carbamazepine (Tegretol) Comprehensive Approach:</b><br>• <b>Morning of Surgery:</b> GIVE regular morning dose.<br>• <b>NPO Bridging Phase:</b> ⚠️ <b>No direct IV formulation available.</b> If prolonged NPO, consult neurology to bridge with <b>IV Levetiracetam or Phenytoin</b>.</div>", unsafe_allow_html=True)
            if "Lamotrigine" in aed:
                st.markdown("<div class='specialty-box'>🧠 <b>Lamotrigine (Lamictal) Comprehensive Approach:</b><br>• <b>Morning of Surgery:</b> GIVE morning dose.<br>• <b>NPO Bridging Phase:</b> ⚠️ <b>No direct IV formulation available.</b> Long half-life allows short holding window (<24h), resume orally as soon as bowel functions return.</div>", unsafe_allow_html=True)
            if "Phenytoin" in aed:
                st.markdown("<div class='danger-box' style='background-color: #FFF5F5; border-color: #E53E3E; color: #9B2C2C;'>🧠 <b>Phenytoin (Epanutin) Comprehensive Approach:</b><br>• <b>Morning of Surgery:</b> GIVE regular morning dose.<br>• <b>NPO Bridging Phase:</b> Convert to <b>IV Phenytoin at a 1:1 ratio</b>.<br>• ⚠️ <i>CRITICAL SAFETY:</i> Infuse slowly (< 50 mg/min) and only mixed with Normal Saline. Monitor ECG.</div>", unsafe_allow_html=True)
...
# ==============================================================================
# TAB 5: GLOBAL CLEARANCE SUMMARY DASHBOARD
# ==============================================================================
with tabs[4]:
    ...
    if potassium > 5.2 or potassium < 3.5:
        ...
        mandatory_actions.append("🧪 Correct serum Potassium immediately to safe zone (3.5 - 5.0 mEq/L).")

    # --- AUTOMATED THYROID TRIGGERS ---
    if tsh_level > 4.5:
        if ft4_level < 0.8:
            clinical_issues.append(f"Severe Overt Hypothyroidism (TSH: {tsh_level} µIU/mL, Low FT4: {ft4_level}).")
            mandatory_actions.append("❌ <b>HOLD ELECTIVE SURGERY:</b> Delay surgery until optimized on Levothyroxine to avoid refractory hypotension.")
        else:
    ...
    elif tsh_level < 0.45 and (ft4_level > 1.8 or ft4_level > 4.2):
        clinical_issues.append(f"Overt Hyperthyroidism (TSH: {tsh_level} µIU/mL, High FT4/FT3).")
        mandatory_actions.append("❌ <b>HOLD ELECTIVE SURGERY:</b> Postpone immediately to prevent intraoperative <b>Thyroid Storm</b>.")
    ...
    if last_seizure in ["Seizure within the last 6 months", "Seizure within the last 30 days"]:
        ...
        mandatory_actions.append("🧠 Order emergency serum AED drug levels. Ensure IV Benzodiazepines are present in the OR.")
    elif last_seizure == "Seizure-free for > 6 months":
        clinical_issues.append("History of Controlled Epilepsy.")
        mandatory_actions.append("🧠 Maintain threshold: Ensure morning oral AED dose is given. Utilize 1:1 IV bridging post-op if NPO.")
    ...
    if any("❌" in act or "HOLD" in act for act in mandatory_actions):
        ...
        st.success("🟢 CLEARANCE STATUS: CLEARED WITH STANDARD SYSTEMIC PRECAUTIONS - Internally stable.")
