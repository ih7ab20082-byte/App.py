...
# Main Grid Split
col_labs, col_logic = st.columns([1, 1.2])
...
with col_labs:
    ...
    # Calculate steady state Cr clearance (CrCl) using Cockcroft-Gault as standard proxy for kGFR equations
    gender_factor = 0.85 if p_sex == "Female" else 1.0
    baseline_crcl = ((140 - p_age) * p_weight) / (72 * cr_baseline) * gender_factor
    
    # Fractional change in creatinine
    delta_cr = cr_current - cr_initial
    mean_cr = (cr_initial + cr_current) / 2
    
    # Kinetic clearance multiplier formula
    # Max potential plasma Cr accumulation rate is ~2.0-2.5 mg/dL per day if GFR=0 (depends on volume of distribution, assumed ~0.6 * weight)
    # The mathematical formula captures the dynamic divergence from the steady state
    kgfr_multiplier = 1 - ((24 * delta_cr) / (time_delta * (mean_cr * 1.5))) # Clinical approximation of rapid shift
    ...
    # Base insensible loss: 10-12 mL/kg/24h. For every degree > 37°C, add 10-13% or roughly 2.5 mL/kg/day per degree.
...
with col_protocol:
    ...
    with tab_kgfr:
        ...
        # Calculate standard static eGFR using baseline Cockcroft-Gault for comparison
        ...
        # Clinical Risk Stratification based on direction of kGFR vs Static Clearance
        ...
        if kgfr < 15:
            ...
            st.markdown("🟢 **Normal Range Dosing:** Clearance is stable enough for standard empiric dosing. Re-evaluate if creatinine fluctuates.")
    ...
    with tab_fluid:
        ...
        with col_f3:
            st.metric("Net Daily Fluid Balance", f"{net_fluid_balance:+.0f} mL", 
                      delta=f"Positive Balance" if net_fluid_balance > 0 else "Negative Balance",
                      delta_color="inverse" if net_fluid_balance > 300 else "normal")
        ...
        # Net Fluid Balance Recommendations
        if net_fluid_balance > 500:
            ...
            * **Stewardship Action:** Assess perfusion markers (JVP, Mucous membranes, Capillary refill). Consider gentle volume resuscitation or decreasing diuretic dosages.
            """)
        else:
