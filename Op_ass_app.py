...
# 2. Custom Enhanced CSS for Professional Clinical Interface
st.markdown("""
    <style>
...
# 3. App Header & Professional Branding Block
...
# 4. Main Application Tabs Definition
tabs = st.tabs([
...
with tabs[0]:
    ...
    if spo2 < 92:
        vitals_alerts.append(f"🔴 CRITICAL HYPOXEMIA DETECTED: Preoperative SpO2 is dangerously low ({spo2}%). Elective surgery is strictly contraindicated. Assess for acute pulmonary edema, severe COPD/Asthma exacerbation, or PE. Secure airway and oxygenate immediately.")
    if sbp >= 180 or dbp >= 110:
        vitals_alerts.append(f"🔴 CRITICAL HYPERTENSION DETECTED: Blood pressure is severely elevated ({sbp}/{dbp} mmHg). Delay elective surgeries until stabilized. For urgent cases, coordinate with anesthesia for immediate IV antihypertensives.")
    if sbp < 90:
        vitals_alerts.append(f"🔴 CRITICAL HYPOTENSION DETECTED: Systolic blood pressure is dangerously low ({sbp} mmHg). Risk of hypoperfusion. Investigate for sepsis, occult bleeding, or severe dehydration. Optimize with fluid resuscitation before any surgical induction.")
    if temp >= 38.0:
        vitals_alerts.append(f"🔴 PREOPERATIVE PYREXIA / FEVER: Patient temperature is {temp}°C. This indicates an active inflammatory or infectious process. Elective surgeries should be postponed until infection source is identified and treated.")
    if hr >= 120 or hr < 40:
        vitals_alerts.append(f"🔴 CRITICAL ARRHYTHMIA WARNING: Heart rate is abnormal ({hr} bpm). High risk for perioperative myocardial ischemia or acute heart failure. Obtain emergency 12-lead ECG.")
...
with tabs[1]:
    ...
    if chest_ausc == "Basal Bilateral Crepitations (Rales)" or jvp == "Elevated JVP (Sign of Volume Overload)" or cxr_findings == "Pulmonary Congestion / Cephalization" or edema:
        synthesis_alerts.append("⚠️ ACTIVE PULMONARY/SYSTEMIC CONGESTION: Clinical markers (Rales, JVP, Edema, or CXR finding) indicate hypervolemia/heart failure decompensation. Consider administering IV Diuretics (Furosemide) preoperatively and monitoring input/output strictly.")
    if chest_ausc == "Diffuse Wheezing":
        synthesis_alerts.append("⚠️ ACTIVE BRONCHOSPASM (Wheezing): High risk for bronchospasm during endotracheal intubation. Administer pre-op bronchodilator nebulizers and consider systemic steroids.")
...
with tabs[2]:
    ...
    with col_r2:
        ...
        # Safe Mapping for ARISCAT Points Computation
        p_score += {"≥ 96% (0 pts)": 0, "91 - 95% (8 pts)": 8, "≤ 90% (24 pts)": 24}.get(p_spo2_str, 0)
        p_score += 17 if p_resp else 0
        p_score += 11 if p_anemia else 0
        p_score += {"Peripheral (0 pts)": 0, "Upper Abdominal (15 pts)": 15, "Intrathoracic (24 pts)": 24}.get(p_site_str, 0)
...
# ==============================================================================
# TAB 6: GLOBAL RISK SUMMARY DASHBOARD (Pure HTML Rendering Engine)
# ==============================================================================
with tabs[5]:
    ...
    # Render Dashboard Card with No Mixed Syntax Conflicts
    summary_html = f"""
    <div class='summary-card'>
        <h3 style='color:#0F766E; margin-top:0px; margin-bottom:5px;'>Patient Clearance Summary Report</h3>
        <p style='margin:0px; color:#6B7280; font-size:13px;'>Al-Ahli Hospital Internal Medicine Department</p>
        <hr style='margin-top:10px; margin-bottom:15px; border:0; border-top:1px solid #E5E7EB;'>
        
        <p style='margin-bottom:8px;'><strong>Patient Identifier:</strong> {patient_name} | <strong>Age:</strong> {patient_age} years old</p>
        <p style='margin-bottom:8px;'><strong>Scheduled Procedure:</strong> {op_type} ({op_duration} hours expected duration)</p>
        <p style='margin-bottom:15px;'><strong>Preoperative Oxygenation Baseline:</strong> SpO2 {spo2}% on Room Air</p>
        
        <div style='background-color:#F9FAFB; padding:12px; border-radius:6px; margin-bottom:15px;'>
            <h4 style='color:#0F766E; margin-top:0px; margin-bottom:8px;'>1. Cardiovascular Risk Status</h4>
            <ul style='margin:0px; padding-left:20px; line-height:1.5;'>
                <li><strong>Revised Cardiac Risk Index (RCRI) Score:</strong> {rcri_score} Points</li>
                <li><strong>Estimated Risk of MACE:</strong> {estimated_risk}</li>
                <li><strong>Functional Capacity (METs):</strong> {mets_capacity}</li>
            </ul>
        </div>
        
        <div style='background-color:#F9FAFB; padding:12px; border-radius:6px; margin-bottom:15px;'>
            <h4 style='color:#0F766E; margin-top:0px; margin-bottom:8px;'>2. Pulmonary Risk Status</h4>
            <ul style='margin:0px; padding-left:20px; line-height:1.5;'>
                <li><strong>ARISCAT Score:</strong> {p_score} Points</li>
                <li><strong>Stratification Tier:</strong> {ppc_class}</li>
            </ul>
        </div>
        
        <div style='background-color:#F9FAFB; padding:12px; border-radius:6px; margin-bottom:5px;'>
            <h4 style='color:#0F766E; margin-top:0px; margin-bottom:8px;'>3. Anesthesia & Systemic Baseline</h4>
            <ul style='margin:0px; padding-left:20px; line-height:1.5;'>
                <li><strong>Assigned Global Status:</strong> {asa_class}</li>
                <li><strong>Preoperative Hemoglobin:</strong> {hb} g/dL | <strong>Serum Potassium:</strong> {potassium} mEq/L</li>
            </ul>
        </div>
    </div>
    """
    ...
    # 5. Dynamic Diagnostics & Conflict Evaluation Table
    st.markdown("<h4 style='color:#0F766E;'>Dynamic Clinical Alerts & Recommendations Table</h4>", unsafe_allow_html=True)
    
    all_clinical_issues = []
    
    # Check Vitals Input
    if sbp >= 180 or dbp >= 110:
        all_clinical_issues.append("<b>Severe Hypertension:</b> BP is critical. Recommend postponing elective surgery. Optimize with oral/IV medications.")
    if sbp < 90:
        all_clinical_issues.append("<b>Severe Hypotension:</b> Risk of circulatory shock. Recommend immediate IV fluid challenges, septic workup, and assessment of volume status before clearance.")
    if temp >= 38.0:
        all_clinical_issues.append(f"<b>Preoperative Fever ({temp}°C):</b> Suspected active infection. Delay elective surgery; check CBC, CRP, and cultures to avoid worsening sepsis intraoperatively.")
    if spo2 < 92:
        all_clinical_issues.append("<b>Critical Hypoxemia:</b> SpO2 < 92%. Absolute hold for elective cases. Secure oxygenation and request urgent ABG/Pulmonary consultation.")
    if hr >= 120 or hr < 40:
        all_clinical_issues.append("<b>Hemodynamic Dysregulation (Heart Rate):</b> Rule out acute coronary syndrome or uncontrolled arrhythmia. Order emergency 12-lead ECG.")
        
    # Check Physical Exam & Imaging Inputs
    if cv_ausc in ["Severe Aortic Stenosis", "Systolic Murmur (Possible Aortic Stenosis)"] or echo_valvular == "Severe Aortic Stenosis":
        all_clinical_issues.append("<b>Valvular Pathology (Aortic Stenosis suspected/confirmed):</b> High risk of severe hemodynamics drops during anesthesia. Cardiology clearance via official Echo evaluation is strictly mandatory.")
    if chest_ausc == "Basal Bilateral Crepitations (Rales)" or jvp == "Elevated JVP (Sign of Volume Overload)" or cxr_findings == "Pulmonary Congestion / Cephalization" or edema:
        all_clinical_issues.append("<b>Volume Overload / Decompensated Heart Failure Status:</b> Patient has signs of fluid retention. Administer pre-op IV Furosemide, optimize LVEF medications, and ensure a net-negative/even fluid balance target.")
    if chest_ausc == "Diffuse Wheezing":
        all_clinical_issues.append("<b>Active Airway Hyperreactivity (Bronchospasm):</b> Ensure rescue inhalers are optimized. Add short course steroids if necessary to prevent perioperative severe bronchospasm.")
        
    # Check Labs Input
    if hb < 7.0:
        all_clinical_issues.append("<b>Severe Anemia (Hb < 7 g/dL):</b> Requires mandatory PRBCs transfusion support before surgical incision to optimize myocardial oxygen delivery.")
    if potassium > 5.2 or potassium < 3.5:
        all_clinical_issues.append(f"<b>Electrolyte Derangement (Potassium: {potassium} mEq/L):</b> Correct strictly to 3.5-5.0 mEq/L window immediately to eliminate ventricular fibrillation risks during induction.")

    # Render Conditional Logic Alerts Container
    if all_clinical_issues:
        issues_html = "<div class='danger-box'><strong>⚠️ Identified Clinical Conflicts & Optimization Rules:</strong><ul style='margin-top:5px; padding-left:20px;'>"
        for issue in all_clinical_issues:
            issues_html += f"<li style='margin-bottom:6px;'>{issue}</li>"
        issues_html += "</ul></div>"
    ...
    # 6. Conditional Final Clearance Text Verdict
    st.markdown("<h4 style='color:#0F766E;'>Final Medical Fitness Recommendation</h4>", unsafe_allow_html=True)
    if spo2 < 92 or sbp >= 180 or sbp < 90 or temp >= 38.0 or (rcri_score >= 1 and mets_capacity == "No (< 4 METs)") or potassium > 5.2 or potassium < 3.5 or hb < 7.0 or cv_ausc == "Severe Aortic Stenosis" or echo_valvular == "Severe Aortic Stenosis":
        st.markdown("<div class='danger-box' style='font-size:15px;'>❌ <strong>CLEARANCE STATUS: CONDITIONAL HOLD / HIGH RISK</strong><br>The patient presents with significant or active clinical deviations (as highlighted in the alerts above). Elective surgery should be deferred until the respective optimization parameters are fully corrected and stabilized.</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div class='status-box' style='border-left: 5px solid #16A34A; background-color: #F0FDFA; font-size:15px;'>🟢 <strong>CLEARANCE STATUS: CLEARED FOR SURGERY WITH STANDARD PRECAUTIONS</strong><br>The patient is stable from an Internal Medicine perspective. Proceed with the planned procedure. Ensure specific home chronic disease medications are managed as outlined in the Medication Stewardship protocol.</div>", unsafe_allow_html=True)
