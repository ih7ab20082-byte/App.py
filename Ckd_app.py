...
with col_input:
    ...
    # 3. 24-Month Kidney Failure Risk Equation (KFRE) 4-Variable North American Equation
    # Logit = 4.3601 - 0.5484*(eGFR/5) + 0.0436*(Age/5 for specific centers but standard 4-var coefficient is based on raw metrics)
    # Standardized 4-variable coefficients for 2-year risk:
    sex_coef = 0.2201 if p_sex == "Male" else 0
    kfre_logit = (
        -0.2201 * sex_coef 
        - 0.5484 * (egfr / 5) 
        + 0.0436 * (p_age / 5) 
        + 0.6130 * (st.math.log(u_acr) if u_acr > 0 else 0)
    ) # Note: Simplified validation template coefficient for UI presentation
...
with col_protocol:
    ...
    if egfr >= 50:
        ...
        st.write("* **Metformin:** <span style='color:red; font-weight:bold;'>CONTRAINDICATED!</span> Hold immediately to prevent Lactic Acidosis.", unsafe_allow_html=True)
