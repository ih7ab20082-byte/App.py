...
with col_input:
    ...
    with st.expander("🔋 Core Ions Assessment", expanded=True):
        ...
        potassium = st.number_input("Serum Potassium (mEq/L):", min_value=1.5, max_value=8.0, value=4.2, step=0.1)
...
with col_output:
    ...
    # ------------------------------------------------------------------
    # INSULIN & POTASSIUM SAFEGUARDS (CRITICAL MEDICAL BRAIN)
    # ------------------------------------------------------------------
    st.markdown("---")
    ...
    if potassium < 3.3:
        ...
        <div class='critical-box'>
            <strong>🚫 ABSOLUTE INSULIN CONTRAINDICATION ACTIVE!</strong><br>
            Serum Potassium is critically low (<strong>{potassium} mEq/L</strong>).<br>
            • <strong>HOLD ALL INSULIN.</strong> Giving insulin now will shift potassium intracellularly, risking fatal cardiac arrhythmias.<br>
            • Infuse 20-30 mEq K+ per hour until Serum K+ rises above 3.3 mEq/L.
        </div>
        """, unsafe_allow_html=True)
    ...
    dka_pool = [
        {
            "id": 1,
            "case": "A 24-year-old female presents with DKA. Initial labs: Glucose 420 mg/dL, pH 7.12, Serum Potassium 3.1 mEq/L, Anion Gap 22. The medical resident is preparing to hang a regular insulin infusion infusion.",
